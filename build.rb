...
BUILD_DIR= File.absolute_path("#{DEPLOY_DIR}/tmp/madek")
...
def pack build_archive
  exec! <<-CMD.strip_heredoc
    ...
    cd #{BUILD_DIR}
    cd ..
    tar cfz #{DEPLOY_DIR}/#{build_archive} madek
  CMD
  print "packed, ..."
end
...
def main
  ...
  FileUtils.ln_s build_archive, "Madek.tar.gz", force: true
  print "linked, ..."
  puts " done "
end
